...
npm run scrape         # node lib/scrape.mjs — Cambodia scrape → rates.xlsx (CLI, not the server)
npm run json           # lib/scrape.mjs --json  (raw JSON to stdout)
npm run payload        # lib/scrape.mjs --payload  (the /rates Power-Automate payload shape)
```

There is **no test suite / linter config**. Verify changes by running the server and hitting the endpoints (see below). Probe scripts (ad-hoc `.mjs` that import `lib/providers.mjs`) are the usual way to check a provider's live API.
...
- **Client components must not import `lib/ranking.mjs`, `lib/manual.mjs`, `lib/scrape.mjs`, or `lib/cache.mjs`** (they use `node:fs`). Import pure data/helpers from `lib/countries.mjs` instead; inline small constants (e.g. FILL colors) if needed.
- **`lib/theme.mjs` + `lib/meta.mjs` are load-bearing, despite looking legacy.** The live app imports `lib/ranking.mjs`/`lib/analytics.mjs` for their data functions, and those still `import` from `lib/theme.mjs` (→ `lib/meta.mjs`) at the top level — so the HTML-renderer halves come along for the ride. Don't delete `lib/theme.mjs`/`lib/meta.mjs` or the render exports without first stripping those imports. (The truly-legacy `server.mjs`/`dashboard.mjs` and the host-launch `.bat`s were removed in the Docker-only cleanup — recover from git history if ever needed.)
