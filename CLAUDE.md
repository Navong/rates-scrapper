...
**Cache + warmer: `lib/cache.ts` + `lib/redis.ts`.** In-memory singletons pinned to `globalThis` are the hot path every request reads. Policy is stale-while-revalidate: serve cached instantly, refresh in the background. A **background warmer** (started once via `instrumentation.ts`) refreshes one corridor at a time on a rotation — upstream load is corridors×time, never users×time, and this is what keeps GME under its burst limit. **Last-known-good carry-forward:** if a provider fails a scrape but a value from the last `MAX_STALE` exists, it's carried and its "unavailable" warning is cleared (matching both `PROVIDER` and `PROVIDER/METHOD` error shapes).

**Persistence + multi-instance (Redis):** with `REDIS_URL` set, ALL persistent state is in Redis and **shared across instances**: the corridor cache (`rank:<code>`), manual rates (`manual`), fee overrides (`fees`), and analytics (`events` list). Exactly one instance runs the warmer via a Redis **leader lock** (`warmer:leader`); followers serve the shared copy and never duplicate-scrape (on-demand scrapes are guarded by a per-corridor lock `lock:scrape:<code>`). `manual.ts`/`fees.ts` keep their **synchronous** getters (`getStore`/`feeFor`) by holding an in-memory copy hydrated from Redis at boot (`initManual`/`initFees`, called from `instrumentation.ts`) and kept fresh across instances via pub/sub (`manual:changed`/`fees:changed`) — no async cascade into `ranking.ts`. **Redis is optional and degrades gracefully:** no `REDIS_URL` → in-memory + the `STATE_DIR` JSON files, single instance (the historical behavior); Redis down at runtime → every helper catches and falls back to single-instance behavior. Existing JSON files are auto-migrated into Redis on first boot (file → seed the Redis key).
...
- **Warmer runs on exactly one instance.** With Redis, that's enforced by the leader lock (safe to run multiple app instances — followers won't scrape). WITHOUT Redis it's still one-instance-only (every process would warm + fight over GME). Either way, only one process can bind `:8787` locally — check `netstat -ano | findstr :8787` when the port seems "stale"; a Docker container from an old image can shadow the host build.
