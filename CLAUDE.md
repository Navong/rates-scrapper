...
**Persistence + multi-instance (Redis):** with `REDIS_URL` set, the corridor cache is durable in Redis (`rank:<code>` keys) and **shared across instances**. Exactly one instance runs the warmer via a Redis **leader lock** (`warmer:leader`); followers serve the shared Redis copy and never duplicate-scrape (on-demand scrapes are guarded by a per-corridor lock `lock:scrape:<code>`). **Redis is optional and degrades gracefully:** no `REDIS_URL` → in-memory + `STATE_DIR/rank-cache.json` file, single instance (the historical behavior); Redis down at runtime → every helper catches and falls back to single-instance behavior. **NOTE (Phase 1):** only the rate cache is on Redis so far — `manual.ts`/`fees.ts`/`analytics.ts` still use their `STATE_DIR` JSON files (Phase 2). So today it's still effectively single-instance for manual/fee edits until those move too.
